...
│   ├── 林富强+TASK8.pdf      # 最终提交用 PDF（封面+目录+摘要+四章正文+附录）
│   ├── 林富强+TASK8.docx     # Word 源文件
│   ├── README.md             # TASK8 说明文件
│   ├── code/                 # 代码模块
│   │   ├── build_task8_report.py  # 报告生成：图表 → docx → PDF（Word COM）
│   │   └── charts.py         # 图表绘制模块（中文标题、红涨绿跌、统一风格）
│   └── images/               # 图片资源（共 8 张）
│       ├── fig1_path.png ~ fig8_task7.png  # 图 1–8：学习路径/均线/海龟/机器学习/截面/实战衰减
└── README.md
```
...
> 2026-07-24：TASK8 综合学习报告完成

- **任务**：成果展示——根据导师对 TASK1–7 的反馈进行针对性改进，并综合七次任务撰写规范学习报告（封面 / 目录 / 摘要 / 四章正文 / 附录）
- **素材来源**：全部数据取自 TASK1–7 真实回测与模型结果（均线 / 海龟 / 机器学习 / 截面 / 实战），导师反馈要点已吸收进正文与附录
- **报告结构**：
  - 第一章 量化交易核心概念（本质界定 + 核心价值：纪律性 / 可回测 / 可复制 / 风险可控）
  - 第二章 策略综合分析（均线交叉 / 海龟突破 / 机器学习选股 / 平台实战对比 + 多策略组合思路）
  - 第三章 机器学习应用总结（预处理 / 特征工程 / 模型 / 评估 / 趋势与局限）
  - 第四章 结论与展望
  - 附录：8 条针对性改进建议（建议一 ~ 建议八），正文按编号引用
- **图表**：图 1–图 8（学习路径 / 均线超额收益 / 海龟三标的 / 海龟 24 组热力图 / ML 两数据集 AUC / 截面年化收益 / 截面 IC / 实战样本外衰减），表 1–表 5；全文统一中文编号、风格一致、红涨绿跌、无英文变量名
- **格式**：宋体 / 五号 / 1.5 倍行距 / 0 段距 / 两端对齐，已固化到 `code/build_task8_report.py`
- **生成方式**：本机 Anaconda + Word 直接复现（图表脚本 → docx → Word COM 转 PDF），目录在导出时由 Word 填入真实页码
- **提交**：以 `林富强+TASK8.pdf` 提交（提交后不可修改，已核对目录页码与图表清晰度）
